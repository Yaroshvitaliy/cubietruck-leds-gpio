...
const createInvalidPinErrMsg = (message, pin) => 
    `${message}. Wrong pin provided: '${pin}'. Possible values: ${GPIO_CONSTANTS.GPIO_MIN_PIN} to ${GPIO_CONSTANTS.GPIO_MAX_PIN}`; 

const isDirectionValid = (direction) => 
    [GPIO_CONSTANTS.GPIO_IN_DIRECTION, GPIO_CONSTANTS.GPIO_OUT_DIRECTION]
    .includes(direction);

const createInvalidDirectionErrMsg = (message, direction) =>
    `${message}. Wrong direction provided: '${direction}'. Possible values: ${GPIO_CONSTANTS.GPIO_IN_DIRECTION}, ${GPIO_CONSTANTS.GPIO_OUT_DIRECTION}`;

const isPinValueValid = (value) =>
    [GPIO_CONSTANTS.GPIO_OFF_VALUE, GPIO_CONSTANTS.GPIO_ON_VALUE]
    .includes(value);

const createInvalidValueErrMsg = (message, value) =>
    `${message}. Wrong value provided: '${direction}'. Possible values: ${GPIO_CONSTANTS.GPIO_OFF_VALUE}, ${GPIO_CONSTANTS.GPIO_ON_VALUE}`;
...
export const getPinDirection = async (pin) => {
    ...
    if (data.includes(GPIO_CONSTANTS.GPIO_IN_DIRECTION)) {
        return GPIO_CONSTANTS.GPIO_IN_DIRECTION;
    }

    if (data.includes(GPIO_CONSTANTS.GPIO_OUT_DIRECTION)) {
        return GPIO_CONSTANTS.GPIO_OUT_DIRECTION;
    }
...
export const setPinDirection = async (pin, direction) => {
    ...
    if (!isDirectionValid(direction)) {
        throw new Error(createInvalidDirectionErrMsg(`Failed to set a direction for a pin: '${pin}'`), pin);
    }
...
export const getPinValue = async (pin) => {
    ...
    if (data.includes(GPIO_CONSTANTS.GPIO_OFF_VALUE)) {
        return GPIO_CONSTANTS.GPIO_OFF_VALUE;
    }

    if (data.includes(GPIO_CONSTANTS.GPIO_ON_VALUE)) {
        return GPIO_CONSTANTS.GPIO_ON_VALUE;
    }
